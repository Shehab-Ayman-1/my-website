...
export default function Header() {
	...
	useEffect(() => {
		...
		let systemBtns =
			document.querySelectorAll("header .system-section .theme-btn") ||
			document.querySelectorAll("header .system-section .font-btn");
		...
		<header>
			...
			<nav className={`header-navbar ${window.screenX > 992 ? "show-right-clip" : ""}`}>
				<div className="navbar-title">
					<img src="./images/logo.png" alt="img-logo" className="navbar-img" />
				...
				<div className="system-theme">
					...
					<div className="system-themes-list">
						{themes.map((theme, i) => {
							return (
								<p
									className={`theme-btn light-${theme}-theme`}
									key={i}
									data-theme={theme}
									onClick={toggleTheme}>
									{`light ${theme} theme`}
